...
public class LoggingAspect {
	...
	@Around("execution(public int indi.sword.spring.aop.ArithmeticCalculator.*(..))")
	public Object aroundMethod(ProceedingJoinPoint pjd){
		...
		try {
			//前置通知
			System.out.println("aroundMethod.before...The method " + methodName + " begins with " + Arrays.asList(pjd.getArgs()));
			//执行目标方法
			result = pjd.proceed();
			//返回通知
			System.out.println("aroundMethod.returning...The method " + methodName + " ends with " + result);
		} catch (Throwable e) {
			//异常通知
			System.out.println("aroundMethod.throwing...The method " + methodName + " occurs exception:" + e);
